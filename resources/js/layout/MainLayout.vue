<template>
    <!-- <Link href="/listing">Listing Page</Link>&nbsp; -->
    <!-- <Link href="/listing/create">Create Page</Link> -->

    <header
        class="border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 w-full"
    >
        <div class="container mx-auto">
            <nav class="p-4 flex items-center justify-between">
                <div class="text-lg font-medium">
                    <Link :href="route('listing.index')">Listings</Link>
                </div>
                <div
                    class="text-xl text-indigo-600 dark:text-indigo-300 font-bold text-center"
                >
                    <Link :href="route('listing.index')">LaraZillow</Link>
                </div>
                <div class="text-lg font-medium">
                    <Link :href="route('listing.create')" class="btn-primary"
                        >+ New Listing</Link
                    >
                </div>
            </nav>
        </div>
    </header>
    <main class="container mx-auto p-4">
        <div
            v-if="postSuccess"
            class="mb-4 border rounded-md shadow-sm border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900 p-2"
        >
            {{ postSuccess }}
        </div>
    </main>
    <!-- auto unwrap timer object -->
    <!-- <div>this is the timer: {{ timer }}</div>
    <div>this is the timer * 2: {{ drivenY }}</div> -->

    <slot>Default</slot>
</template>

<script setup>
import { Link, usePage } from "@inertiajs/vue3";
import { ref, computed } from "vue";

const timer = ref(0); // this is a object, like state in react
setInterval(() => timer.value++, 1000);
const drivenY = computed(() => timer.value * 2);

// page.props.value.flash.success
const page = usePage();

const postSuccess = computed(() => page.props.flash.success);
</script>
